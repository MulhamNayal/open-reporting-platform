...
backend/
  ...
  Services/{Area}/    one service per feature (Reports, Datasets, DataSources, ReportPages, Widgets)
  Models/             EF entities
...
- **Frontend (Vitest + React Testing Library):** `npm run verify` before calling any change done. RTL's automatic cleanup isn't enabled (no Vitest globals) — every test file needs its own `afterEach(cleanup)`.
