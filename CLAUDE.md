...
- **The deploy applies EF migrations to the target database itself** — `deploy.yml` generates an `--idempotent` script from the `Migrations/` folder into the bundle, and `deploy-bootstrap.ps1` runs it (via `apply-sql-script.ps1`) *before* stopping the app pools, so a migration failure aborts the deploy with the old build still serving. Nothing migrates at startup. Don't hand-write a migration script into `scripts/sql/` for a deploy — adding the migration is enough, and a committed script is a second source of truth that will drift. (The `migrate-reportingdb*.sql` files there predate this and are historical.)
