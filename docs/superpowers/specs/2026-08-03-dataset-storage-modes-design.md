...
**Decided 2026-08-03.** Two databases, on the same SQL Server instance:

| Database | Holds | Lifecycle |
|---|---|---|
| `ReportingDb` | reports, pages, widgets, bindings, datasets, connections | EF Core migrations; must be backed up |
| `ReportingCacheDb` | one table per Import dataset, in a `mat` schema | created and dropped at runtime; fully derived, never backed up |

The split test is simply: *could this be regenerated by re-running a query?* Yes → cache database.
No → application database. Same shape Metabase and Superset use, for the same reasons — different
backup policy, different growth profile, and the application needs `CREATE TABLE`/`DROP TABLE`
rights on the cache database that it should not hold over its own definitions.

Materialised tables are named `mat.Dataset_{id}`.

> **Deployment note.** A company-owned database called `ReportingDb` already exists on the shared
> staging instance. These names are unambiguous on a private instance but would collide there —
> the deployed environment needs either a different instance or a prefix (`OrpReportingDb` /
> `OrpCacheDb`).
...
1. ~~Where does the platform database live?~~ **Resolved 2026-08-03** — `ReportingDb` +
   `ReportingCacheDb` on the same instance; see *Where materialised tables live* above. Still worth
   noting the staging server has 16.5 GB free of 450 GB, so the 43 GB untruncated
   `IQIADMIN-STAGING` log should be dealt with before materialising there.
